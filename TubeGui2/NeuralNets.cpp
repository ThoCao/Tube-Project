...
void NeuralNets::clearResult_PERCENT(){
	FAILED_EDGE = 0;
	PASSED_EDGE = 0;
	FAILED_INNER = 0;
	PASSED_INNER = 0;

	/* defect postion on edge */
	this->isEdgeFailed = false;
}
	...
	for (auto contour : _circle_edges) {
		...
		for (auto index = 0; index < max_index; index++) {
			...
			for (auto id = start; id + step_size < end; id += step_size) {
				...
				if (id > contour.contour.size() - 1) break;
			...
			if (setPoints.size() == FEATURE_SIZE) {
				...
				if (failed_val > 0.88) {
					...
					/* Save to flag */
					this->isEdgeFailed = true;
					this->edgePos = setPoints[setPoints.size() / 2];
					this->edgeRadius = cv::norm(setPoints[0] - setPoints.back());
...
bool NeuralNets::getResultInner(cv::Mat & _image){
	...
	if (failed_val > 0.88) isDefect = true;
