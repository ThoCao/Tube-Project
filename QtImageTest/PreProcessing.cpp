...
std::tuple<Circle, Circle,bool> PreProcessing::Find2CirclesFunc(const cv::Mat & input){
	...
	if (big_circles.size() > 0 && small_circles.size() > 0) {
		...
		//// cal curvature
		//std::string outter_curve = "outer30.csv";
		//std::string inner_curve  = "inner30.csv";
		//
		//const int curvature_size = 30;
		//calCurvature(big_circles[0], curvature_size, outter_curve);
		//calCurvature(small_circles[0], curvature_size, inner_curve);
